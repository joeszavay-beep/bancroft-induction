-- =====================================================================
-- CoreSite RLS Lockdown Migration
--
-- WHAT THIS DOES:
-- 1. Creates helper functions for operative auth (get_my_operative_id, get_operative_company_id)
-- 2. Creates RPC functions for public flows (QR sign-in, snag reply, aftercare, portal, toolbox sign)
-- 3. Drops ALL existing permissive policies on ALL tables
-- 4. Creates new restrictive policies scoped by company_id for authenticated users
-- 5. Zero anon access to any table — public flows use RPC functions
--
-- PRE-REQUISITES:
-- - get_my_company_id() function must exist (for manager company scoping)
-- - All operatives who need QR access must have Supabase Auth accounts
-- - Storage bucket policies should be locked down FIRST (#2)
--
-- ROLLBACK: If anything goes wrong, the entire transaction rolls back.
--           No partial state. Run the old rls-policies.sql to restore.
--
...
-- Get the operative_id from the authenticated user's JWT metadata
-- Returns NULL for managers, anon users, or users without operative_id
...
-- Get the company_id for the authenticated operative
-- Returns NULL for managers, anon users, or if operative not found
...
-- =====================================================================
-- PART 2: RPC FUNCTIONS FOR PUBLIC FLOWS
-- These replace direct table access for unauthenticated/anon flows.
-- Each function is SECURITY DEFINER so it bypasses RLS internally.
-- =====================================================================

-- 2a. QR Sign-In: load project branding before operative logs in
...
-- 2b. Snag Reply: load a single snag by its reply token
...
-- 2c. Snag Reply: submit a reply (update snag + add comment)
...
-- 2d. Aftercare: get defects for a specific project + email
...
DECLARE
  ...
  v_company_id uuid;
BEGIN
  -- Look up company_id from the project (don't trust client input)
...
-- 2g. Toolbox: load talk info for signing
...
BEGIN
  ...
  -- Get talk for company_id and project validation
  ...
  -- Verify operative is assigned to the project this talk belongs to
  IF NOT EXISTS (
...
-- Grant EXECUTE on RPC functions to anon (they're the public callers)
...
-- Also grant to authenticated (operatives may call these too)
