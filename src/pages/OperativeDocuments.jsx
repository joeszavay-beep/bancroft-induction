...
import { ArrowLeft, FileText, CheckCircle2, Clock, Lock } from 'lucide-react'
...
export default function OperativeDocuments() {
  ...
  const signedDocIds = new Set(signatures.map(s => s.document_id))
        ...
        {documents.length === 0 ? (
          ...
          <div className="space-y-2">
            {documents.map((doc, idx) => {
              ...
              const isLocked = !isSigned && documents.slice(0, idx).some(d => !signedDocIds.has(d.id))
              ...
              return (
                ...
                >
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                    isSigned ? 'bg-success/10' : isLocked ? 'bg-navy-700' : 'bg-accent/10'
                  }`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`font-medium truncate ${isSigned ? 'text-gray-400' : 'text-white'}`}>{doc.title}</p>
                    <p className="text-xs text-gray-500">
                      {isSigned ? 'Signed' : isLocked ? 'Complete previous documents first' : 'Tap to review & sign'}
                    </p>
                  </div>
                </button>
              )
