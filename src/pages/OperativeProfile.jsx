    ...
    <div className="min-h-dvh bg-navy-950 flex flex-col">
      <header className="bg-navy-900 border-b border-navy-700 px-4 py-3 flex items-center gap-3 shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-lg font-bold text-white truncate">My Profile</h1>
          <p className="text-xs text-gray-400 truncate">{operative?.name}</p>
        </div>
      </header>
      ...
      <form onSubmit={handleSave} className="flex-1 p-4 space-y-4 overflow-y-auto pb-8">
        {!isComplete && (
          <div className="bg-warning/10 border border-warning/30 rounded-xl p-4 flex items-start gap-3">
            ...
            <div>
              <p className="text-sm text-white font-semibold">Complete your profile</p>
              <p className="text-xs text-gray-400 mt-1">Please fill in all fields below. This information is required for site induction compliance.</p>
            </div>
          </div>
        )}
        ...
        <div className="bg-navy-800 border border-navy-600 rounded-xl p-4 space-y-4">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h3 className="text-sm font-semibold text-gray-300">Personal Details</h3>
          </div>
          ...
          <div>
            <label className="text-xs text-gray-400 mb-1 block">National Insurance Number *</label>
            <input
              value={niNumber}
        ...
        <div className="bg-navy-800 border border-navy-600 rounded-xl p-4 space-y-4">
          ...
          <div>
            <label className="text-xs text-gray-400 mb-1 block">Home Address *</label>
            <textarea
              value={address}
              onChange={e => setAddress(e.target.value)}
              placeholder="Full address including postcode"
              rows={3}
              className="w-full px-4 py-3 bg-navy-700 border border-navy-600 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-accent resize-none"
            />
          </div>
        </div>
        ...
        <div className="bg-navy-800 border border-navy-600 rounded-xl p-4 space-y-4">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h3 className="text-sm font-semibold text-gray-300">Emergency Contact / Next of Kin</h3>
          </div>

          <div>
            <label className="text-xs text-gray-400 mb-1 block">Next of Kin Name *</label>
            <input
              value={nextOfKin}
          ...
          <div>
            <label className="text-xs text-gray-400 mb-1 block">Next of Kin Phone *</label>
            <input
              type="tel"
        ...
        <LoadingButton loading={saving} type="submit" className="w-full bg-accent hover:bg-accent-dark text-white text-lg py-4">
          Save Profile
        </LoadingButton>
      </form>
    </div>
