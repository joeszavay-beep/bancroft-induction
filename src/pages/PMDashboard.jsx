...
import {
  ...
  Mail, Settings, Bell, ShieldCheck, ClipboardList, ArrowLeft,
              ...
              <div key={p.id} className="rounded-xl border overflow-hidden" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-color)' }}>
                ...
                {expanded && (
                  <div className="border-t p-5 space-y-4" style={{ borderColor: 'var(--border-color)' }}>
                    {/* Documents (general only — RAMS live in the Risk Assessments section) */}
                    <div>
                      <ProjectDocumentsSection
                        project={p}
                        docs={projDocs.filter(d => (d.doc_type || 'general') !== 'rams')}
                        ...
                        onRefresh={onRefresh}
                      />
                      {projDocs.some(d => d.doc_type === 'rams') && (
                        <p className="flex items-center gap-1.5 text-[11px] mt-2" style={{ color: 'var(--text-muted)' }}>
                          <ShieldCheck size={12} className="shrink-0" />
                          {projDocs.filter(d => d.doc_type === 'rams').length} risk assessment{projDocs.filter(d => d.doc_type === 'rams').length !== 1 ? 's' : ''} for this project {projDocs.filter(d => d.doc_type === 'rams').length !== 1 ? 'are' : 'is'} managed in the <a href="/app/rams" className="underline hover:opacity-70" style={{ color: 'var(--primary-color)' }}>Risk Assessments</a> section (included in the sign-off figures above)
                        </p>
                      )}
                    </div>
...
function HSReportTab() {
  const navigate = useNavigate()
  useEffect(() => { navigate('/app/hs-reports', { replace: true }) }, [])
  return null
}
