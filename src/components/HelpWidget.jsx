...
import { X, Send, Loader2, HelpCircle, ArrowRight, ChevronDown } from 'lucide-react'
...
export default function HelpWidget() {
  ...
  return createPortal(
    <div id="coresite-help-widget">
      {/* Chat panel */}
      {open && (
        <div className="fixed inset-0 sm:inset-auto sm:bottom-20 sm:right-4 z-[60] transition-all duration-300 animate-in fade-in slide-in-from-bottom-4">
          <div className="flex flex-col bg-white shadow-2xl shadow-black/20 overflow-hidden w-full h-full sm:w-[400px] sm:h-[560px] sm:rounded-2xl rounded-none border border-slate-200/50">

            {/* Header — CoreSite branded */}
            <div className="shrink-0" style={{ background: 'linear-gradient(135deg, #1A2744 0%, #1B6FC8 100%)' }}>
              <div className="px-5 py-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  {/* CoreSite crosshair icon */}
                  ...
                  <div>
                    <h2 className="text-white font-semibold text-[14px] tracking-wide">CoreSite Help</h2>
                    <p className="text-white/50 text-[11px]">Ask me anything about the platform</p>
                  </div>
                </div>
                <button
                ...
                <div className="space-y-3">
                  {messages.map((msg, i) => (
                    <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} gap-2`}>
                      {msg.role === 'assistant' && (
                        <div className="w-7 h-7 rounded-full shrink-0 flex items-center justify-center mt-0.5" style={{ background: 'linear-gradient(135deg, #1A2744 0%, #1B6FC8 100%)' }}>
                  ...
                  {loading && (
                    <div className="flex justify-start gap-2">
                      <div className="w-7 h-7 rounded-full shrink-0 flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #1A2744 0%, #1B6FC8 100%)' }}>
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round">
                          <circle cx="12" cy="12" r="2" fill="white" />
                        </svg>
                      </div>
      ...
      {/* Floating button — CoreSite branded */}
      <button
