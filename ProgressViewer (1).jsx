...
export default function ProgressViewer() {
  ...
  const moveCursor = (e) => {
    ...
    // Position the cursor synchronously — straight to the DOM, no frame delay and no re-render
    const el = cursorRef.current
    if (el) { el.style.transform = `translate3d(${x}px, ${y}px, 0)`; el.style.opacity = '1' }
    // The paste ghost preview needs a re-render to follow the cursor — throttle to once per frame, paste mode only
    if (clipboard) {
      cursorXY.current = { x, y }
      if (!cursorRaf.current) {
        cursorRaf.current = requestAnimationFrame(() => {
          cursorRaf.current = null
          setPastePos({ x: cursorXY.current.x, y: cursorXY.current.y })
        })
      }
    }
  }
