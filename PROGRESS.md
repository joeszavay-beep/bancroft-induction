...
- **§2.16 / §2.16b — ✅ CLOSED + LIVE-VERIFIED 2026-06-28 (PR #42).** Live-DB trace of the "cross-tenant `pm_email`" item reframed it and surfaced a bigger bug:
  - **§2.16 → LOW (inert), retired:** the cross-tenant clobber was real (`settings.UNIQUE(key)`) but `pm_email` had **no consumer**. All 4 read/write sites removed (0 code refs remain).
  - **§2.16b → HIGH (NEW, customer-affecting), FIXED:** company **sign-off email notifications were non-functional end-to-end** — three disconnected fields; the send path read a `companies.notification_email` **column that didn't exist**, the error was swallowed, recipient never resolved, **no email ever sent**. Option A shipped: added the real `companies.notification_email` column (migration applied + verified in prod; security-gated `api/notify` open-relay check untouched), repointed both settings UIs, retired `pm_email`, and surfaced the swallowed errors (api/notify 500 / SignDocument console.error). **Owner ran the real-inbox test on the deployed app — a sign-off completion email actually ARRIVED (first time ever).** `RESEND_API_KEY` confirmed set in prod. Regression guard logged as **TH-4**. Full detail: **AUDIT.md §2.16 / §2.16b**.
