  ...
  function exportCSV() {
    ...
    const rows = records.map(r => {
      ...
      return [
        ...
        r.method || '',
        r.ip_address || '',
        (r.latitude && r.longitude) ? `${r.latitude},${r.longitude}` : (r.notes?.includes('Location off') ? 'Location off' : ''),
      ]
    })
                  ...
                  <tr key={r.id} className="border-b last:border-0 hover:opacity-80"
                    ...
                    <td className="py-2.5 px-3">
                      <div className="flex items-center gap-1.5">
                        ...
                        {r.notes?.includes('Location off') && <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-amber-50 text-amber-700">LOCATION OFF</span>}
                    ...
                    {historyWithDuration.map(r => (
                      <tr key={r.id} className="border-b last:border-0" style={{ borderColor: 'var(--border-color)' }}>
                        ...
                        <td className="py-2 px-3 hidden lg:table-cell text-xs">
                          ...
                          ) : r.notes?.includes('Location off')
                            ? <span className="font-semibold text-amber-600">Location off</span>
