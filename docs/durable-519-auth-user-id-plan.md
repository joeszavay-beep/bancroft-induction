...
| **PR4 dual-accept** ✅ **MERGED + DEPLOYED + E2E-green + smoke-verified 2026-06-21 (PR #17 → main `d4d78d7`)** | 3 helpers → COALESCE(auth.uid arm first, interim fallback) — `get_my_company_id` 3-arm (profiles first); `resolve_login_route`/`operative_exists_by_email`/`get_operative_public_info` already left_at-guarded in PR3b (no PR4 SQL change). Worker-login sites (`PMLogin`/`OperativeLogin`/`SiteSignIn`) resolve active record by `auth_user_id` **with email fallback** (PR5 hard-switches). Capture-first: live==repo (no drift) → dry-run ROLLBACK → COMMIT → re-capture verified. SQL applied live; code deploys via Vercel on merge (SQL already live → no exposure window). **Manual smoke** (icloud worker→Thomas Worley via auth.uid; manager→Bancroft) + E2E `operative-dual-accept.spec` both paths; full `RLS_LOCKDOWN_APPLIED=1` suite **52 passed**. Safety proven on live data (0 tenant redirects). SQL: `rls-5-19-pr4-dual-accept.sql`. | **yes** (fn bodies + code) | `rls-5-19-pr4-dual-accept-rollback.sql` (captured interim defs verbatim) |
| **PR5 enforce** ✅ **MERGED + DEPLOYED + smoke-verified 2026-06-21 (PR #19 → main `718f2c8`)** | 3 helpers → auth.uid()+`left_at` only (interim arm DROPPED; `get_my_company_id` keeps profiles-first + auth.uid operative arm); 3 binding sites → `auth_user_id`-only (PR4 email fallback removed); `create-operative-account.js` stops writing `user_metadata.operative_id`. **Capture-first:** live `pg_get_functiondef` == PR4 dual-accept *semantically* but *cosmetically* drifted from the repo apply file (no comments / 4-space indent) → rollback = the **verbatim capture**. **Part B re-proven FRESH = 0** (no active+unlinked operative shares an email with an auth account → nobody locked out). dry-run `ROLLBACK` (no-metadata/email 0, collisions 0, Part B 0) → `COMMIT` → re-capture verified auth.uid()+left_at only. **E2E `operative-enforce.spec` 3/3** + full suite **52 passed**. **Smoke-verified live** (icloud worker → Thomas Worley via the deployed auth_user_id-only binding, matching the live SQL). **DECOUPLED (owner):** `UNIQUE(lower(email))` → optional PR5b only (active-only `WHERE left_at IS NULL`, non-load-bearing hygiene, gated on dup-check + ordering audit); §5.20 → separate follow-up (constraint now lifted). SQL: `rls-5-19-pr5-enforce.sql` | **yes** (fn bodies + code) | `rls-5-19-pr5-enforce-rollback.sql` (verbatim dual-accept capture) |
