...
| **PR5 enforce** — ⏭️ LAST step, plan-first, **NOT urgent** (interim fallback is redundant-but-harmless until then; auth.uid() already resolves every linked operative) | 3 helpers → auth.uid()+`left_at` only (drop interim arm); hard-switch the 3 binding sites to `auth_user_id`-only (drop the PR4 email fallback); stop writing `user_metadata.operative_id`; `UNIQUE(lower(email))` forward-guard; close §5.20 path. Gate (all met): dual-accept verified ✓, both E2E paths green ✓, collisions=0 ✓ | **yes** (fn bodies + code) | re-apply PR4 (dual-accept) defs |
