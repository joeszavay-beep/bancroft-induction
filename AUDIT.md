...
### 5.7c [BLOCKING GATE] Agency search+connect UNVERIFIED in the locked state — verify before the first agency onboards
...
- **R1 [OPEN — product/process gap, NOT an RLS bug]: nothing promotes an agency from `pending_verification` → `active`.** Confirmed by grep across `src/`, `api/`, `scripts/`: agencies register as `pending_verification` (`Signup.jsx:195`, `AgencyRegister.jsx:75`) and `search_agencies` returns only `status='active'`, but **no UI, endpoint, cron, or RPC ever flips the status** (the `agencies` UPDATE policy exists, but the app never offers a status change). So a real agency registers and is **permanently undiscoverable** until someone runs manual SQL. **Interim process:** a super-admin runs `UPDATE agencies SET status='active' WHERE id=…` to verify a new agency. **Decision deferred (owner):** (i) document the manual-SQL process, or (ii) build a small super-admin verify toggle mirroring the `set-manager-active` endpoint pattern. To handle before the first agency onboards.
