...
- **FOLLOW-UP (next item, NOT vague backlog) — in-app nav blocker:** `beforeunload` only fires for real browser unloads (tab close / reload / external nav), **not** in-app React-Router route changes (clicking the sidebar) — the most common mid-edit exit. React-Router v7's `useBlocker` would cover it but **throws outside a data router**, and the app uses `<BrowserRouter>`/`<Routes>` (`main.jsx:53`), so adding it requires migrating the router to `createBrowserRouter`/`RouterProvider` (app-wide, every route). Deliberately deferred to its own change so it doesn't balloon/endanger this CRIT data-loss fix. Once the data-router migration is done, add a `useBlocker` confirm on `dirty` in `ProcurementScheduler` (and reuse for any other autosave page). Visible indicator (1) is the primary defense meanwhile.
...
- **FOLLOW-UP (deliberate prod SQL, owner-applied):** add `UNIQUE(job_id, operative_id, date, is_manual_entry)` to `timesheet_entries` so a double-count is structurally impossible (the second auto insert would error, not duplicate). Safe to add — data confirmed clean. Pending owner confirmation that **multiple manual entries per operative/day is not a valid workflow** (if it is, use a partial unique on auto rows only). After the constraint is live, the client clear-then-insert is belt-and-braced by the DB; an optional `.upsert` swap is **not** strictly cleaner (pure upsert drops the "operative no longer in this week's QR" removal that the delete handles).
...
### 2.16 [HIGH] Global `settings.pm_email` row is not company-scoped — cross-tenant overwrite
- **File:** `src/pages/CompanySettings.jsx:343-347`; `src/pages/PMDashboard.jsx:1415`, `:1429-1432`
- **Issue:** `upsert({ key: 'pm_email', value }, { onConflict: 'key' })` writes one **global** row. Every company's save overwrites every other company's notification email, and the read returns whichever tenant wrote last — worker sign-off notifications can go to the wrong company. Upsert error also unchecked.
- **Fix:** Add `company_id` to the table with `UNIQUE(company_id, key)` and scope reads/writes — or drop this legacy path in favour of `companies.settings` JSONB.
