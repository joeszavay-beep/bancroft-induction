...
- **CLOSED 2026-06-28 (PR #42).** All 4 `pm_email` read/write sites removed; both settings UIs now use the per-company `companies.notification_email` column. The cross-tenant `settings.pm_email` row is no longer read or written by the app (`grep pm_email src/ api/` → 0 code refs). Closed together with §2.16b.
...
- **CLOSED — LIVE-VERIFIED 2026-06-28 (PR #42, `companies.notification_email` migration applied + verified in prod).** Owner ran the real-inbox test on the deployed app: set a company notification email, triggered a sign-off, and the completion email **actually arrived** — the first time this feature has ever delivered. Option A shipped (real column; send path/open-relay gate untouched; both settings UIs repointed; dead `pm_email` retired; swallowed errors now surface — api/notify 500 / SignDocument console.error). `RESEND_API_KEY` confirmed set in prod (welcome emails arrive). §2.16 (inert `pm_email`) retired in the same PR. Regression-guard E2E logged as **TH-4**.
...
### 5.23 [LOW] `document_signoffs` RLS is authenticated-only (unused `company_id` column) — and the hub sign-off flow is half-built (no signing surface)
- **Files:** `scripts/migrations/rls-lockdown.sql:835-838` (`ds_select`/`ds_insert`/`ds_update`/`ds_delete` all gate on bare `auth.role() = 'authenticated'`); `src/pages/DocumentHub.jsx` (the only app writer — inserts `status='pending'` on assignment, updates to `'invalidated'` on supersession); `scripts/seed-hs-report-demo.js` (the only source of `status='signed'` rows in prod).
- **Issue (found 2026-07-02 during the read-only RAMS-section investigation):** the four `document_signoffs` policies are authenticated-only, NOT company-scoped — any logged-in user of ANY tenant can read/write every tenant's sign-off rows, even though the table has its own `company_id` column (populated by DocumentHub inserts, ignored by policy). Compounding: no app surface ever sets `status='signed'` — the operative signing flow for hub documents was never built, so hub sign-offs can only ever be pending/invalidated through the app. **Live blast radius today: demo data only** — all 346 rows (246 `'signed'`) belong to ABC Construction Ltd seed data; no real customer has hub sign-off rows (verified live, service-role, 2026-07-02).
- **Decision (owner, 2026-07-02):** the dedicated Risk Assessment section deliberately does NOT adopt this path — RAMS sign-offs live on `documents` + `signatures` (properly company-scoped for both manager and operative arms; working SignDocument flow). The H&S report RAMS register repoints from `document_hub`/`document_signoffs` to `documents`/`signatures` as part of that feature.
- **Fix (backlog, before the hub sign-off feature ever goes real):** scope all four policies with `company_id = get_my_company_id()` (plus an operative arm if operatives get a hub signing surface), and only then build the operative signing flow that writes `status='signed'`. Until then the table stays demo-only and non-load-bearing.
...
- **TH-4 — Sign-off notification wiring E2E (guards §2.16b).** This feature was silently broken end-to-end for the app's entire life; a permanent guard prevents a silent re-break. Add a wiring-level spec: set a test company's `companies.notification_email`, then POST `/api/notify` with a signed operative from that company → assert **200** (recipient resolves + gates pass; locally hits the "logged, no RESEND" success path), and a mismatched-`to` POST → assert **403** (open-relay gate). Real Resend delivery can't be CI-asserted, but the wiring (the part that broke) can.
