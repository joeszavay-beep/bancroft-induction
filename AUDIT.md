...
- **CONFIRMED BY E2E (2026-06-09):** `e2e/auth.spec.js` test `[KNOWN-RED: AUDIT §1.7]` sets only the `pm_auth` flag (no Supabase session) and navigates to `/app/plant-equipment`; the app stays on that URL instead of redirecting to `/login`, proving the guard admits a dead session. Left **red** until fixed.
...
- **CONFIRMED BY E2E (2026-06-09):** `e2e/plant.spec.js` reproduces this live. Create persists fine; editing serial `SN-001 → SN-999` (plus hire company + rate) through the real UI, then re-fetching the row from Supabase, shows the DB still holds `SN-001` — the edit was silently dropped exactly as described. The test is intentionally left **red** as a regression marker (titled `[KNOWN-RED: AUDIT §2.1]`) until this is fixed.
